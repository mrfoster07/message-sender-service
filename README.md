...
- Также поле хэша сообщения тоже лишнее, и индекс на нем, это понадобится для проверки на повторную обработку сообщения из очереди, когда запись в БД будет вынесена паралельно.
